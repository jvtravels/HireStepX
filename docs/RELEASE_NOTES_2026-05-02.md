...
**Range**: `ee4c8ae..5ed7ee3` (8 commits)
**Date**: 2026-05-02
**Theme**: Closing the audit punch-list before public launch — security, data integrity, observability, test coverage, UX polish.

> **Operator action required before this reaches prod.** See the
> "Deploy checklist" at the bottom — three env vars + one schema
> migration that must land before traffic hits the new code.
...
> **Verification will fail-closed in production until items 1 and 4
> are done.** That's the safe-fail behavior, not a regression.
