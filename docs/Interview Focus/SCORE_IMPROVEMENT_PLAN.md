...
### ✅ Phase 5 — Stretch (\~1.5d, +0.2) — **DONE** (Session A `5b21dc8` + Session B `ec7d29e`)
...
- **6.1** ✅ **Cross-session adaptive prebias for behavioural.** Mirrored the existing HR-round `priorFlags → PRIOR_FLAG_TO_DIMENSION → prompt directive → telemetry` chain into the behavioural focus. `getLatestSessionInsightFlags(userId, focus?)` now accepts an optional focus filter so a behavioural session pulls only the latest behavioural insights (and vice-versa). `useInterviewEngine` gates the prebias fetch on `new Set(["hr-round", "behavioral"])` and forwards `interviewFocus`. `generate-questions` adds `BEHAVIORAL_PRIOR_FLAG_TO_DIMENSION` covering all 9 canonical analyzer flags (weak STAR, missing result, we-attribution, metric-without-baseline, unevidenced metric, accepted vague, no learning, unquantified) and renders a `LAST-SESSION COACHING PRIORITIES` directive into the prompt template — gated on `interviewType === 'behavioral'` to prevent cross-firing. New PostHog counters `behavioral_prior_coverage_hints` + `behavioral_prior_coverage_applied`. 9 source-pattern-matching tests in `behavioralPriorCoverage.test.ts` pin every link of the chain.
- **6.2** ✅ **Answer ↔ question topical-alignment detector.** New `server-handlers/_topical-alignment.ts` pure helper combines two signals: (a) intent-tag classifier across 9 behavioural intents (conflict / failure / leadership / ambiguity / feedback / decision-making / mentorship / metric-impact / deadline-pressure) — the question is tagged, the answer scanned for any matching signal; (b) content-token overlap (stopword-filtered, ≥3-char, deduped). An answer is flagged off-topic ONLY when the question has a clear intent AND the answer carries none of it AND token overlap is ≤1 — all three must agree. Wired into `behavioral.ts`: each user answer pairs with the immediately preceding AI turn; ≥2 misses across the session emits the `answer_off_topic` flag + rubric gaps + coaching note. Map entry added to `BEHAVIORAL_PRIOR_FLAG_TO_DIMENSION` so next session's question generator prebiases toward prompt-anchored stems. 15 unit tests pin positive detections + conservative skips (no question intent / intent match / overlap ≥2).
...
- **6.6** ✅ **Designer-affinity bank coverage** for Senior Product Designer loops. Added 5 entries to `data/behavioral-question-bank.ts` (`cnf-06` PM/eng design disagreement, `amb-05` unclear-problem-clarity-through-design, `dec-06` UX vs business goal, `fdb-04` changed-design-direction-after-critique, `mnt-05` raised-team-design-quality), loosened `cnf-03` + `mnt-02` to include `designer`. Pre-Phase-6.6 the bank had zero designer-affinity entries — every designer fell through to universal questions and missed the SPD-specific influence / judgement / leadership shape. Bank size 56 → 61. 2 new tests pin the 5 canonical SPD ids + the cross-competency designer coverage floor.
- **6.6b** ✅ **Company-specific theme bias.** Added structured `themes?: string[]` field to `KnownFacts` (`data/company-known-facts.ts`) — renders as an imperative `BIAS QUESTIONS TOWARD: …` line inside the question-generation prompt. Populated for Meesho (9 themes covering the 7 audit-canonical biases: India scale, Tier 2/3 buyers, mobile-first, low-bandwidth, social-commerce behaviour, seller↔customer trust, growth & retention loops), Flipkart, Swiggy, Zomato, PhonePe, Razorpay. 11 new tests pin (a) schema discipline, (b) renderer behaviour (skips empty / absent), (c) Meesho audit-canonical coverage, (d) peer-company minimum theme count.
