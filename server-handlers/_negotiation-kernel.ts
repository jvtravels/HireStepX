...
function backfillCandidateProfile(raw: unknown): CandidateProfileResult {
  ...
  return {
    ...
    earlySwitcher: v?.earlySwitcher ?? false,
    lowCtcAlert: v?.lowCtcAlert ?? false,
    ...
    peopleManagementClaimed: v?.peopleManagementClaimed ?? false,
    crossBorderAnchor: v?.crossBorderAnchor ?? false,
    unvestedEquityLossClaim: v?.unvestedEquityLossClaim ?? false,
    explodingOfferPressure: v?.explodingOfferPressure ?? false,
    postAcceptanceRenege: v?.postAcceptanceRenege ?? false,
    quotaAttainmentClaimed: v?.quotaAttainmentClaimed ?? false,
    gardenLeaveDisclosed: v?.gardenLeaveDisclosed ?? false,
    nonCompeteFlagged: v?.nonCompeteFlagged ?? false,
    relocationBonusAsked: v?.relocationBonusAsked ?? false,
    parentInsuranceAsked: v?.parentInsuranceAsked ?? false,
    inHandTakehomeFocus: v?.inHandTakehomeFocus ?? false,
    rtoPushback: v?.rtoPushback ?? false,
    returnshipMaternity: v?.returnshipMaternity ?? false,
    payBandAsked: v?.payBandAsked ?? false,
    taxStructureAsked: v?.taxStructureAsked ?? false,
    bgvAnxiety: v?.bgvAnxiety ?? false,
    esopSophisticationProbe: v?.esopSophisticationProbe ?? false,
    spouseJobConstraint: v?.spouseJobConstraint ?? false,
    agingParentCare: v?.agingParentCare ?? false,
    moonlightingDisclosed: v?.moonlightingDisclosed ?? false,
    mentalHealthDisclosed: v?.mentalHealthDisclosed ?? false,
    payParityAsked: v?.payParityAsked ?? false,
    preemptiveCounterReceived: v?.preemptiveCounterReceived ?? false,
    acceptanceTimeRequest: v?.acceptanceTimeRequest ?? false,
    cryptoTokenComp: v?.cryptoTokenComp ?? false,
    gccArbitrageAnchor: v?.gccArbitrageAnchor ?? false,
    benchTimeDisclosed: v?.benchTimeDisclosed ?? false,
    founderSecondInnings: v?.founderSecondInnings ?? false,
    latecareerAgeBias: v?.latecareerAgeBias ?? false,
    titlePrecisionAsk: v?.titlePrecisionAsk ?? false,
    currentCtcRefusal: v?.currentCtcRefusal ?? false,
    pregnancyDisclosed: v?.pregnancyDisclosed ?? false,
    boomerangRehire: v?.boomerangRehire ?? false,
    referralReceived: v?.referralReceived ?? false,
    hometownReturnPreference: v?.hometownReturnPreference ?? false,
    pwdDisability: v?.pwdDisability ?? false,
    gratuityVestingNear: v?.gratuityVestingNear ?? false,
    acquisitionContextAsk: v?.acquisitionContextAsk ?? false,
    lgbtqDisclosure: v?.lgbtqDisclosure ?? false,
    chronicIllnessDisclosed: v?.chronicIllnessDisclosed ?? false,
    noticeBuyoutAsk: v?.noticeBuyoutAsk ?? false,
    bfsiClawbackContext: v?.bfsiClawbackContext ?? false,
    bigFourGradeStep: v?.bigFourGradeStep ?? false,
    securityClearanceNeeded: v?.securityClearanceNeeded ?? false,
    missionDrivenComp: v?.missionDrivenComp ?? false,
    edtechReputationCheck: v?.edtechReputationCheck ?? false,
    acquiHireContext: v?.acquiHireContext ?? false,
    cabinParkingAsk: v?.cabinParkingAsk ?? false,
    spanOfControlAsk: v?.spanOfControlAsk ?? false,
    preResignationStealth: v?.preResignationStealth ?? false,
    reverseAnchorAsk: v?.reverseAnchorAsk ?? false,
    ...
    salaryReviewCadenceAsk: v?.salaryReviewCadenceAsk ?? false,
    multipleOffersJuggling: v?.multipleOffersJuggling ?? false,
    recruitmentAgencyMediation: v?.recruitmentAgencyMediation ?? false,
    internalTransferContext: v?.internalTransferContext ?? false,
    offerRescindedHistory: v?.offerRescindedHistory ?? false,
    internationalDegreePremium: v?.internationalDegreePremium ?? false,
    domesticTopMbaAnchor: v?.domesticTopMbaAnchor ?? false,
    toxicManagerContext: v?.toxicManagerContext ?? false,
    visaSponsorshipNeed: v?.visaSponsorshipNeed ?? false,
    casteReservationContext: v?.casteReservationContext ?? false,
    veteranTransition: v?.veteranTransition ?? false,
    singleParentConstraint: v?.singleParentConstraint ?? false,
    jointFamilyFinancialResp: v?.jointFamilyFinancialResp ?? false,
    paternityLeaveAsk: v?.paternityLeaveAsk ?? false,
    menstrualLeavePolicy: v?.menstrualLeavePolicy ?? false,
    esopExerciseLoanAsk: v?.esopExerciseLoanAsk ?? false,
    preIpoSecondaryAsk: v?.preIpoSecondaryAsk ?? false,
    accelerationTriggerAsk: v?.accelerationTriggerAsk ?? false,
    esopPerquisiteTaxAsk: v?.esopPerquisiteTaxAsk ?? false,
    tenderOfferCycleAsk: v?.tenderOfferCycleAsk ?? false,
    probationaryDurationAsk: v?.probationaryDurationAsk ?? false,
    offerLetterTurnaroundDemand: v?.offerLetterTurnaroundDemand ?? false,
    contractToHireAsk: v?.contractToHireAsk ?? false,
    headcountApprovalCheck: v?.headcountApprovalCheck ?? false,
    ipAssignmentClauseAsk: v?.ipAssignmentClauseAsk ?? false,
    healthcarePharmaContext: v?.healthcarePharmaContext ?? false,
    manufacturingCoreContext: v?.manufacturingCoreContext ?? false,
    quickCommerceContext: v?.quickCommerceContext ?? false,
    d2cConsumerEquity: v?.d2cConsumerEquity ?? false,
    hasAny: v?.hasAny ?? false,
  };
}
