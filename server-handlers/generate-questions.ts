...
3. CLOSING: A neutral wrap-up template that the runtime swaps in ONLY when the kernel reaches a terminal action (acceptance, walk-away, or stalemate). Two safe sentences: "Thanks for the conversation today. We'll be in touch with next steps." Do NOT invent a final package number, do NOT promise specific numbers, do NOT thank them for "accepting" (they haven't yet). The kernel rewrites this with the real terms when negotiation terminates. CRITICAL — PDF#49 SAFETY: this closing line is RESERVED for kernel-routed terminal turns. The model that runs each turn must NEVER emit closing prose ("we'll be in touch", "next steps soon", "thanks for the conversation today") while the planner is still in discovery, probe, range-disclosure, counter-offer, lever-explore, or any non-terminal phase. A candidate disclosing their current CTC, asking about base/variable, or even saying "the number is low" is NOT a session end — keep the negotiation moving. The post-LLM contract validator will reject and replace any closing-shape prose emitted on a non-terminal turn.
