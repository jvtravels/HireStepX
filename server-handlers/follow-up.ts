...
Repeat-text in followUpText is FORBIDDEN.`;
    ...
    if (isSalaryNeg && parsed.followUpText) {
      ...
      // S83-B1/B2: removed `agreed` (past-tense reference false positive) and guarded `i agree` (point-agreement false positive)
      const acceptRe = /\b(i accept|i.?ll accept|accept the offer|sounds good|that works for me|it.?s a deal|i.?m happy with|fine with me|i agree(?!\s+(?:the|that|this|it|your|with)\b)|let.?s go ahead)\b/i;
