...
Repeat-text in followUpText is FORBIDDEN.`;
    ...
    if (
      ...
      try {
        ...
        // S94-B2: synced deal-closing idioms (done deal, we/you got a deal, i'm game)
        // S95-B1: bare "works for me"; S95-B2: "happy to accept"
        const acceptInHistoryReLocal = /\b(i accept|i.?ll accept|i agree(?!\s+(?:the|that|this|it|your|with)\b)|sounds good|that works for me|works for me|happy\s+to\s+accept|it.?s a deal|happy with|let.?s go ahead|deal|i.?ll take it|i.?ll take the offer|count\s+me\s+in|consider\s+it\s+done|(?:that|this).?s?\s+(?:is\s+)?acceptable|(?:i.?m|i\s+am)\s+on\s+board|fine\s+with\s+me|(?:i.?m|i\s+am)\s+happy\s+to\s+proceed|done\s+deal|(?:we|you)(?:.ve\s+got|\s+have|\s+got)\s+(?:yourself\s+)?a\s+deal|i.?m\s+game)\b/i;
