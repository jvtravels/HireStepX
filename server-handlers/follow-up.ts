...
Repeat-text in followUpText is FORBIDDEN.`;
    ...
    if (isSalaryNeg && parsed.followUpText) {
      ...
      const numberLockWordsLocal = /\b(?:stick(?:ing)?\s+with|hold(?:ing)?\s+(?:at|firm)|stay(?:ing)?\s+at|firm\s+at)(?=[^.]*\b(?:lakh|lpa|crore|cr\b|\d))\b|\bse\s+kam\s+nahi\s+(?:lung[ai]|loong[ai])\b/i;
