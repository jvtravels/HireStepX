...
Repeat-text in followUpText is FORBIDDEN.`;
    ...
    if (
      ...
      try {
        ...
        const acceptInHistoryReLocal = /\b(i accept(?!\s+(?:nothing|no\s+less|anything\s+less))|i.?ll accept|i agree(?!\s+(?:the|that|this|it|your|with|to\s+(?:review|look|discuss|examine|reconsider|revisit|check|read|go|get|think|evaluate|consider))\b)|sounds good|that works for me|works for me|happy\s+to\s+accept|fine\s+by\s+me|fully\s+on\s+board|(?:it|that|this)(?:.?s|\s+is)\s+a\s+deal|happy with|let.?s go ahead|deal|i(?:.ll|.d|\s+will|\s+would)\s+take\s+(?:it|(?:the\s+)?(?:offer|package|deal|position|role))|count\s+me\s+in|consider\s+it\s+done|(?:that|this).?s?\s+(?:is\s+)?acceptable|(?:i.?m|i\s+am)\s+(?:(?:totally|completely|absolutely|definitely|certainly|wholly)\s+)?on\s+board|fine\s+with\s+me|(?:i.?m|i\s+am)\s+(?:(?:very|quite|really|so|truly|absolutely)\s+)?happy\s+to\s+proceed|happy\s+to\s+proceed|done\s+deal|(?:we|you)(?:.ve\s+got|\s+have|\s+got)\s+(?:yourself\s+)?a\s+deal|i.?m\s+game|okay\s+with\s+(?:this|that|it)|happy\s+to\s+(?:take|go\s+with)\s+(?:the\s+)?(?:offer|package|deal|this|that|it)|happy\s+to\s+move\s+forward(?:\s+with\s+(?:this|it|that))?|i(?:.ll|.d|\s+will|\s+would)\s+(?:(?:happily|gladly|certainly|definitely|absolutely|willingly)\s+)?accept(?!\s+(?:nothing|no\s+less))|willing\s+to\s+accept|prepared\s+to\s+accept|glad\s+to\s+accept|i(?:.d|\s+would)\s+love\s+to\s+accept|i\s+(?:wholeheartedly|enthusiastically|unconditionally)\s+accept|i(?:.ll|.d|\s+will|\s+would)\s+go\s+for\s+it|(?:i.?m|i\s+am)\s+(?:(?:totally|completely|absolutely|definitely|certainly|wholly|perfectly|quite)\s+)?fine\s+with\s+(?:this|that|it|the)|sounds\s+acceptable|let(?:'?s|\s+us)\s+(?:proceed|move\s+forward)|(?:offer|package|deal|ctc|salary|comp(?:ensation)?|terms?|arrangement|proposal|number)\s+is\s+acceptable|sounds\s+great(?:\s+to\s+me)?|fair\s+enough(?:\s+for\s+me)?|(?:let.?s|let\s+us)\s+make\s+this\s+happen|(?:i\s+)?can\s+live\s+with\s+(?:that|this|it)|in\s+agreement(?:\s+with\s+the\s+terms)?|i(?:.ll|.d|\s+will|\s+would)\s+go\s+with\s+(?:this|that|it)|(?:i.?m|i\s+am)\s+(?:(?:very|quite|really|so|truly|absolutely|fully|completely)\s+)?satisfied\s+with\s+(?:this|that|it|the)|(?<!not\s)willing\s+to\s+(?:move\s+forward|proceed)|(?:let.?s|let\s+us)\s+do\s+it|i(?:.ll|.d|\s+will|\s+would)\s+go\s+ahead(?:\s+with)?|(?:i.?m|i\s+am)\s+(?:(?:totally|completely|absolutely|perfectly|quite|really|pretty)\s+)?good\s+with\s+(?:this|that|it|the)|consider\s+it\s+settled|happy\s+to\s+sign(?:\s+(?:on|up))?|sign\s+me\s+up|that.?s\s+settled(?:\s+then)?|let.?s\s+shake\s+(?:hands\s+)?on\s+it|(?:i.?m|i\s+am)\s+(?:(?:quite|very|really|truly|completely|absolutely)\s+)?content\s+with\s+(?:this|that|it|the)|(?:i.?m|i\s+am)\s+pleased\s+to\s+accept|(?:i.?ll|i.?d|i\s+will|i\s+would)\s+sign\s+on\s+the\s+dotted\s+line|(?<!not\s)comfortable\s+with\s+(?:this|that|it|the)|works\s+(?:\w+\s+)?for\s+me)\b/i;
