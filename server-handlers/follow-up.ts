...
Repeat-text in followUpText is FORBIDDEN.`;
    ...
    if (isSalaryNeg && parsed.followUpText) {
      ...
      const walkAwayNegationReLocal = /\b(?:don.?t|do\s+not|doesn.?t|does\s+not|not\s+ready\s+to|not\s+going\s+to|never\s+want\s+to|won.?t|not\s+think\s+(?:i|we)\s+need\s+to|was\s+(?:about|going)\s+to)\s+(?:\w+\s+){0,4}(?:walk\s+away|withdraw|part\s+ways|decline|pass|exit)\b/i;
