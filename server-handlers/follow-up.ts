...
`;
      ...
      const acceptInHistoryRe2 = /\b(i accept|i.?ll accept|i agree(?!\s+(?:the|that|this|it|your|with)\b)|sounds good|that works for me|works for me|happy\s+to\s+accept|fine\s+by\s+me|fully\s+on\s+board|it.?s a deal|happy with|let.?s go ahead|deal|i.?ll take it|i.?ll take the offer|count\s+me\s+in|consider\s+it\s+done|(?:that|this).?s?\s+(?:is\s+)?acceptable|(?:i.?m|i\s+am)\s+on\s+board|fine\s+with\s+me|(?:i.?m|i\s+am)\s+happy\s+to\s+proceed|done\s+deal|(?:we|you)(?:.ve\s+got|\s+have|\s+got)\s+(?:yourself\s+)?a\s+deal|i.?m\s+game|okay\s+with\s+(?:this|that|it)|happy\s+to\s+(?:take|go\s+with)\s+(?:the\s+)?(?:offer|package|deal|this|that|it)|happy\s+to\s+move\s+forward(?:\s+with\s+(?:this|it|that))?|i(?:.ll|.d|\s+will|\s+would)\s+accept|willing\s+to\s+accept|let(?:'?s|\s+us)\s+(?:proceed|move\s+forward)|(?:offer|package|deal|ctc|salary|comp(?:ensation)?|terms?|arrangement|proposal|number)\s+is\s+acceptable)\b/i;
...
Repeat-text in followUpText is FORBIDDEN.`;
    ...
    if (
      ...
      try {
        ...
        const acceptInHistoryReLocal = /\b(i accept|i.?ll accept|i agree(?!\s+(?:the|that|this|it|your|with)\b)|sounds good|that works for me|works for me|happy\s+to\s+accept|fine\s+by\s+me|fully\s+on\s+board|it.?s a deal|happy with|let.?s go ahead|deal|i.?ll take it|i.?ll take the offer|count\s+me\s+in|consider\s+it\s+done|(?:that|this).?s?\s+(?:is\s+)?acceptable|(?:i.?m|i\s+am)\s+on\s+board|fine\s+with\s+me|(?:i.?m|i\s+am)\s+happy\s+to\s+proceed|done\s+deal|(?:we|you)(?:.ve\s+got|\s+have|\s+got)\s+(?:yourself\s+)?a\s+deal|i.?m\s+game|okay\s+with\s+(?:this|that|it)|happy\s+to\s+(?:take|go\s+with)\s+(?:the\s+)?(?:offer|package|deal|this|that|it)|happy\s+to\s+move\s+forward(?:\s+with\s+(?:this|it|that))?|i(?:.ll|.d|\s+will|\s+would)\s+accept|willing\s+to\s+accept|let(?:'?s|\s+us)\s+(?:proceed|move\s+forward)|(?:offer|package|deal|ctc|salary|comp(?:ensation)?|terms?|arrangement|proposal|number)\s+is\s+acceptable)\b/i;
