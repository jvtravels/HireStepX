...
const hedgeWords = /\b(but|however|only if|unless|provided|on condition|contingent|except|though|if|as\s+long\s+as|so\s+long\s+as|par\b)\b/i;
...
const shortAffirmativeStart = /^(yes|yeah|okay|ok|sure|deal|agreed|sounds good|that works|fine|absolutely|perfect|haan|hanji|ji\s+haan|ha\s+ji|han\s+ji|theek\s+hai|thik\s+hai|bilkul)\b/i;
...
export function detectCandidateIntent(answer: string): CandidateIntent {
  ...
  const walkAwayNegationRe = /\b(?:don.?t|do\s+not|doesn.?t|does\s+not|not\s+ready\s+to|not\s+going\s+to|never\s+want\s+to|won.?t|not\s+think\s+(?:i|we)\s+need\s+to)\s+(?:\w+\s+){0,4}(?:walk\s+away|withdraw|part\s+ways|decline|pass|exit)\b/i;
  const walkAway = (walkAwayWords.test(trimmed) && !accepted && !walkAwayNegationRe.test(trimmed))
