...
export function detectCandidateIntent(answer: string): CandidateIntent {
  ...
  const rejected = (rejectWords.test(trimmed) && !accepted && !rejNegationRe.test(trimmed) && !(rejectMatchIsWalkAwayArm && walkAwayNegationRe.test(trimmed)) && !notInterestedDoubleNegationRe.test(trimmed))
  ...
  const needsTime = thinkWords.test(trimmed) && (candidateNum === null || numberIsOfferReferenceRe.test(trimmed)) && !thinkNegationRe.test(trimmed);
