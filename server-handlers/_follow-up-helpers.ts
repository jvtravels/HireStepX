...
export function detectCandidateIntent(answer: string): CandidateIntent {
  ...
  const accepted = (hasAccept || isShortAffirmative || isShortAffirmativeConditional) && !hedgeIsRejection && !acceptNegationRe.test(trimmed);
  ...
  const rejected = rejectWords.test(trimmed) && !accepted && !rejNegationRe.test(trimmed) && !walkAwayNegationRe.test(trimmed) && !notInterestedDoubleNegationRe.test(trimmed);
  ...
  const walkAway = (walkAwayWords.test(trimmed) && !accepted && !walkAwayNegationRe.test(trimmed) && !notInterestedDoubleNegationRe.test(trimmed))
    || (hasAnyHedge && walkAwayWords.test(postHedgeText) && !notInterestedDoubleNegationRe.test(postHedgeText));
