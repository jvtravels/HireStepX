...
export function detectCandidateIntent(answer: string): CandidateIntent {
  ...
  const rejected = !thirdPartyDepartureRe.test(trimmed) && (
   ...
   * `accepted`/`rejected`. */
  const walkAway = !thirdPartyDepartureRe.test(trimmed) && (
