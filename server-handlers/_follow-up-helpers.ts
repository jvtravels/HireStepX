...
const hedgeWords = /\b(but|however|only if|unless|provided|on condition|contingent|except|though|if|as\s+long\s+as|so\s+long\s+as|par\b|lekin)\b/i;
...
const deflectWords = /\b(you first|your offer|what.*you.*offer|tell me.*first|don.?t want to share|prefer not|rather not|you tell me|(?:talk|discuss)\s+about\s+\w+(?:\s+\w+){0,3}\s+instead|circle\s+back|come\s+back\s+to\s+this|discuss\s+\w+(?:\s+\w+){0,2}\s+first(?!\s+(?:offer|number|figure|salary|ctc)))\b/i;
...
export function detectCandidateIntent(answer: string): CandidateIntent {
  ...
  const walkAwayNegationRe = /\b(?:don.?t|do\s+not|doesn.?t|does\s+not|not\s+ready\s+to|not\s+going\s+to|never\s+want\s+to|won.?t|not\s+think\s+(?:i|we)\s+need\s+to|was\s+(?:about|going)\s+to)\s+(?:\w+\s+){0,4}(?:walk\s+away|withdraw|part\s+ways|decline|pass|exit)\b/i;
