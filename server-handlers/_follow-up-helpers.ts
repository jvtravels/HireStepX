...
export function detectCandidateIntent(answer: string): CandidateIntent {
  ...
  const numberLockWords = /\b(?:stick(?:ing)?\s+with|stuck\s+with|hold(?:ing)?\s+(?:at|firm)|stay(?:ing)?\s+at|firm\s+at)(?=[^.]*\b(?:lakh|lpa|crore|cr\b|\d))\b|\bse\s+kam\s+nahi\s+(?:lung[ai]|loong[ai]|chalega)\b/i;
