...
const shortAffirmativeStart = /^(yes|yeah|okay|ok|sure|deal|agreed|sounds good|that works|fine|haan|hanji|ji\s+haan|ha\s+ji|han\s+ji|theek\s+hai|thik\s+hai|bilkul)\b/i;
...
export function detectCandidateIntent(answer: string): CandidateIntent {
  ...
  const rejected = rejectWords.test(trimmed) && !accepted && !rejNegationRe.test(trimmed);
  ...
  const thinkNegationRe = /\b(?:don.?t|doesn.?t|do\s+not|does\s+not)\s+(?:need|want)\s+(?:more\s+)?(?:time|to\s+think|to\s+consider|to\s+decide)|no\s+need\s+to\s+(?:think|consider|decide|deliberate)|not\s+time\s+to\s+(?:think|decide|consider)\b/i;
  const needsTime = thinkWords.test(trimmed) && candidateNum === null && !thinkNegationRe.test(trimmed);
