...
const competingWords = /\b(other offer|another offer|another opportunity|competing|another company|other companies|other options|other opportunities|counter.?offer|multiple offers|also talking|also interviewing|also exploring|interviewing at|interviewing with|got an offer|have an offer|received an offer|better\s+offers?\s+elsewhere|(?:also|too)\s+(?:made|gave|offered)\s+me\s+an?\s+offer|offer\s+from\s+(?:a|an|another)\b|weigh(?:ing)?\s+(?:this\s+)?against\s+(?:an\s+)?offer|(?:firm|company|employer)\s+has\s+offered\s+me\s+\d+%\s+more|is\s+offering\s+me\s+more\b|\d+%\s+(?:more|better|higher)\s+offer|bhi\s+(?:mujhe\s+)?offer\s+(?:diya|mila))\b/i;
