 ...
 *   `take\s+(?:some\s+)?(?:more\s+)?time\s+to\s+(?:think|reflect|consider|review|decide|evaluate|process)`.
...
const thinkWords = /\b(need time|think about|think this through|think it through|time to think|time to decide|sleep on|let me think|consider|a\s+(?:day|week|night)(?:\s+or\s+two)?\s+to\s+(?:think|decide|consider|review)|(?:the\s+)?weekend\s+to\s+(?:think|decide|consider|review)|couple\s+of\s+days|(?:a\s+)?few\s+days|give\s+me\s+(?:until|till)|until\s+tomorrow|till\s+tomorrow|until\s+end\s+of\s+(?:the\s+)?(?:week|month|day)|mull\s+(?:(?:it|this|that)\s+)?over|\d+\s+hours?\s+to\s+(?:think|decide|consider|review)|talk to.*(?:spouse|family|partner|wife|husband|parents?|folks?)|(?:check|consult|speak|discuss)\s+with\s+(?:my\s+)?(?:spouse|family|partner|wife|husband|parents?|folks?)|get back to you|not ready|take\s+(?:some\s+)?(?:more\s+)?time\s+to\s+(?:think|reflect|consider|review|decide|evaluate|process)|(?:review|consult|discuss)\s+(?:this\s+)?with\s+(?:my\s+)?(?:lawyer|attorney|advisor|accountant|ca\b|mentor))\b/i;
