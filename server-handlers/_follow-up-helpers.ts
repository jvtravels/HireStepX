...
const shortAffirmativeStart = /^(yes|yeah|okay|ok|sure|deal|agreed|sounds good|that works|fine|absolutely|perfect|haan(?!\s+nahi(?:n)?\b)|hanji|ji\s+haan|ha\s+ji|han\s+ji|theek\s+hai|thik\s+hai|bilkul(?!\s+nahi(?:n)?\b))\b/i;
...
export function detectCandidateIntent(answer: string): CandidateIntent {
  ...
  const rejected = rejectWords.test(trimmed) && !accepted && !rejNegationRe.test(trimmed) && !walkAwayNegationRe.test(trimmed);
