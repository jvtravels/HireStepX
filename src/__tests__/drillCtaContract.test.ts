...
describe("Drill-CTA contract", () => {
  it("every GAP_CTA_MAP entry has a non-empty drill key", () => {
  ...
  it("every drill key has matching prompt guidance in generate-questions DRILL_GUIDANCE", () => {
    ...
    // claims a coached drill but the LLM gets no prompt shaping — the
    // exact silent regression we're guarding against.
    const missing: string[] = [];
    for (const cta of Object.values(GAP_CTA_MAP)) {
      if (!KNOWN_DRILL_GUIDANCE_KEYS.has(cta.drill)) missing.push(cta.drill);
    }
    expect(
      missing,
  ...
  it("drill keys are unique across gap codes (reverse-map is well-formed)", () => {
    ...
    // silently lose one set of banner copy.
