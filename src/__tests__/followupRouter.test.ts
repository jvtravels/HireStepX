...
describe("recommendFollowups — gap-readiness", () => {
  it("fires when gap >= 3 months and activity is not upskill/study", () => {
    const state = baseState({
      candidateProfile: { careerGapMonths: 8, careerGapActivity: "family", tenureSignal: null, levelMismatch: null, domainPivot: false, transferableSkillsClaimed: false, compensationHistoryIssue: null, serviceBondAccepted: false, probationCompMentioned: false, internshipConversion: false, hasAny: true },
    });
  ...
  it("suppresses when activity is upskill", () => {
    const state = baseState({
      candidateProfile: { careerGapMonths: 8, careerGapActivity: "upskill", tenureSignal: null, levelMismatch: null, domainPivot: false, transferableSkillsClaimed: false, compensationHistoryIssue: null, serviceBondAccepted: false, probationCompMentioned: false, internshipConversion: false, hasAny: true },
    });
