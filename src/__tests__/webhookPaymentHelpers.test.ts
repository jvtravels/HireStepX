...
describe("resolveCapturedPayment", () => {
  ...
  describe("full-price subscriptions", () => {
    it("activates Sprint Pack (weekly) at list price ₹39", () => {
      const r = resolveCapturedPayment({ plan: "weekly", amount: 3900, notes: {} });
  ...
  describe("promo-discounted subscriptions", () => {
    it("accepts a discounted Sprint Pack amount when the discount note matches", () => {
      const r = resolveCapturedPayment({ plan: "weekly", amount: 2900, notes: { promo: "SAVE", discount: "1000" } });
      expect(r).toEqual({ kind: "subscription", tier: "starter", planDays: 30 });
    });
    ...
    it("ignores a negative/garbage discount note (collapses to 0)", () => {
      expect(resolveCapturedPayment({ plan: "weekly", amount: 3900, notes: { discount: "-500" } }))
        .toEqual({ kind: "subscription", tier: "starter", planDays: 30 });
      expect(resolveCapturedPayment({ plan: "weekly", amount: 3900, notes: { discount: "abc" } }))
        .toEqual({ kind: "subscription", tier: "starter", planDays: 30 });
    });
  });
  ...
  describe("rejections", () => {
    it("rejects an unknown plan", () => {
      expect(resolveCapturedPayment({ plan: "enterprise", amount: 3900, notes: {} }))
        .toEqual({ kind: "reject", reason: "unknown_plan" });
    });

    it("rejects a missing/non-string plan", () => {
      expect(resolveCapturedPayment({ plan: undefined, amount: 3900, notes: {} }))
        .toEqual({ kind: "reject", reason: "unknown_plan" });
    });

    it("rejects a non-numeric amount", () => {
      expect(resolveCapturedPayment({ plan: "weekly", amount: "3900", notes: {} }))
