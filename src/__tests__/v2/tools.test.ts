...
describe("v2 tools — equity-disclosure trigger (deep-research #12)", () => {
  const baseConcedeState = () =>
    freshState({ hasAnchored: true, lastAnchorLpa: BAND.initialOffer, mentionedNumbers: [32, 4] });

  it("rejects concede(esops) with NO equity_terms", () => {
    const call: ToolCall = {
      name: "concede",
      args: {
        lever: "esops",
        amount_lpa: 4,
        cost_to_company: "above standard grant for this level",
        benefit_to_candidate: "front-loaded upside on the 32 LPA package",
        asked_in_return: "you close at the 32 LPA today",
      },
    };
    const result = executeTool(call, BAND, baseConcedeState());
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toMatch(/equity_terms/i);
  });

  it("rejects concede(esops) with cliff_years < 1 (Companies Act floor)", () => {
    const call: ToolCall = {
      name: "concede",
      args: {
        lever: "esops",
        amount_lpa: 4,
        cost_to_company: "above standard grant for this level",
        benefit_to_candidate: "upside layered on the 32 LPA base",
        asked_in_return: "you close at 32 LPA today",
        equity_terms: { vest_years: 4, cliff_years: 0, strike_or_fmv_note: "strike at last-round valuation" },
      },
    };
    const result = executeTool(call, BAND, baseConcedeState());
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toMatch(/cliff_years.*1 year/i);
  });

  it("rejects concede(esops) with vest_years < 2", () => {
    const call: ToolCall = {
      name: "concede",
      args: {
        lever: "esops",
        ...
        benefit_to_candidate: "upside layered on the 32 LPA base",
        asked_in_return: "you close at 32 LPA today",
        equity_terms: { vest_years: 1, cliff_years: 1, strike_or_fmv_note: "strike at last-round valuation" },
      },
    };
    const result = executeTool(call, BAND, baseConcedeState());
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toMatch(/vest_years/i);
  });

  it("rejects concede(esops) with cliff >= vest (structurally worthless)", () => {
    const call: ToolCall = {
      name: "concede",
      args: {
        lever: "esops",
        ...
        benefit_to_candidate: "upside layered on the 32 LPA base",
        asked_in_return: "you close at 32 LPA today",
        equity_terms: { vest_years: 4, cliff_years: 4, strike_or_fmv_note: "strike at last-round valuation" },
      },
    };
    const result = executeTool(call, BAND, baseConcedeState());
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toMatch(/cliff.*strictly less than vest/i);
  });

  it("rejects concede(esops) with vague strike note", () => {
    const call: ToolCall = {
      name: "concede",
      args: {
        lever: "esops",
        ...
        benefit_to_candidate: "upside layered on the 32 LPA base",
        asked_in_return: "you close at 32 LPA today",
        equity_terms: { vest_years: 4, cliff_years: 1, strike_or_fmv_note: "good upside potential" },
      },
    };
    const result = executeTool(call, BAND, baseConcedeState());
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toMatch(/strike.*FMV|FMV.*last-round|fair-market/i);
  });

  it("ACCEPTS concede(esops) with full equity disclosure — renders all four", () => {
    const call: ToolCall = {
      name: "concede",
      args: {
        lever: "esops",
        ...
        equity_terms: {
          vest_years: 4,
          cliff_years: 1,
          strike_or_fmv_note: "strike at last-round Series C valuation",
        },
      },
    };
    const result = executeTool(call, BAND, baseConcedeState());
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.canonical).toMatch(/4-year vest/);
      expect(result.canonical).toMatch(/1-year cliff/);
      expect(result.canonical).toMatch(/last-round Series C valuation/);
    }
  });

  it("ACCEPTS concede with non-esops lever WITHOUT equity_terms (gate is esops-only)", () => {
    const call: ToolCall = {
      name: "concede",
      args: {
        lever: "joining_bonus",
        amount_lpa: 3,
        cost_to_company: "pulls from joining-bonus pool",
        benefit_to_candidate: "front-loaded cash on the 32 LPA package",
        asked_in_return: "you close at 32 LPA today",
      },
    };
    const result = executeTool(call, BAND, baseConcedeState());
