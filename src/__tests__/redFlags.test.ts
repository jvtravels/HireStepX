  ...
  it("fires when pivoting domain and asking >30% hike", () => {
    const state = baseState({
      ...
      candidateProfile: {
        ...
        compensationHistoryIssue: null, serviceBondAccepted: false, probationCompMentioned: false, internshipConversion: false, hasAny: true,
      },
    });
  ...
  it("fires when delayed-salary recorded on profile", () => {
    const state = baseState({
      candidateProfile: {
        ...
        compensationHistoryIssue: "delayed", serviceBondAccepted: false, probationCompMentioned: false, internshipConversion: false, hasAny: true,
      },
    });
  ...
  it("fires when unpaid-salary recorded", () => {
    const state = baseState({
      candidateProfile: {
        careerGapMonths: null, careerGapActivity: null, tenureSignal: null,
        levelMismatch: null, domainPivot: false, transferableSkillsClaimed: false,
        compensationHistoryIssue: "unpaid", serviceBondAccepted: false, probationCompMentioned: false, internshipConversion: false, hasAny: true,
      },
    });
    expect(codes(state)).toContain("compensation-history-issue");
  });
  ...
  it("fires when candidate raised service-bond on profile", () => {
    const state = baseState({
      candidateProfile: {
        ...
        probationCompMentioned: false, internshipConversion: false, hasAny: true,
      },
    });
  ...
  it("fires when probation comp surfaced", () => {
    const state = baseState({
      candidateProfile: {
        ...
        probationCompMentioned: true, internshipConversion: false, hasAny: true,
      },
    });
