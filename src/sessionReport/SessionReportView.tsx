...
/* ─── Top score drivers panel ────────────────────────────────────────
   "What's actually pulling my score down?" — surfacing the answer in
   a dedicated 3-row panel beats hiding it inside the per-question deep-
   dive. The candidate sees the same severity weight (high=3, medium=2,
   low=1) that the evaluator applied, which makes the score legible
   instead of magical. Drawn from the union of per-question red flags;
   ranked by severity weight, ties broken by appearance order. */
function TopScoreDriversSection({ questions }: { questions: Question[] }) {
  // Flatten every per-question red flag, keep severity + a one-line label.
  ...
  // Stable-sort descending by severity weight; original order preserved for ties.
  const ranked = all
    .map((row, idx) => ({ row, idx }))
  ...
  return (
    <section
      id="ir-section-score-drivers"
      aria-labelledby="ir-score-drivers-heading"
      style={{
        background: t.white,
        border: `1px solid ${t.line}`,
        borderRadius: 16,
    ...
    >
      <SectionEyebrow num="01b" label="Score drivers — what hit hardest" />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 14, flexWrap: "wrap", gap: 12 }}>
        <h2
          id="ir-score-drivers-heading"
          style={{ fontFamily: f.serif, fontSize: "clamp(18px, 4.5vw, 22px)", fontWeight: 400, color: t.coal, margin: 0, letterSpacing: "-0.01em" }}
        >
          Top 3 score drivers
        </h2>
      ...
      <p style={{ fontFamily: f.sans, fontSize: 13, color: t.inkSoft, margin: "0 0 16px", lineHeight: 1.5 }}>
        Each flag is weighted by severity (high = 3, medium = 2, low = 1). These three account for most of the score gap.
      </p>
