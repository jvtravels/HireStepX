...
const PagesResponsiveSheet = () => (
  <style>{`
    ...
    @media (max-width: 880px) and (min-width: 641px) {
      ...
      .mv2p-page-hero { padding-top: 40px !important; padding-bottom: 32px !important; }
...
/* ════════════════════════════════════════════════════════════════════
   METHODOLOGY PAGE
   E-E-A-T trust page: how interview guides, salary bands, and practice
   questions are sourced, labeled, and reviewed. Linked from every
   company/salary/questions page's evidence labels and the footer, per
   the SEO brief's §13 (scaled hiring/comp content needs a stated
   editorial standard, not just a data disclaimer buried per-page).
   ════════════════════════════════════════════════════════════════════ */
export function MethodologyV2() {
  return (
    <PageShell>
      <PageHero
        eyebrow="Methodology"
        title="How we source and label"
        accent="everything we publish."
        lead="HireStepX publishes interview guides and salary data at scale across 300+ companies. That only stays trustworthy if every claim is labeled by how we know it, not presented as more certain than it is."
        narrow
      />

      <section className="mv2p-section" aria-label="Editorial standards" style={{ ...sectionBase }}>
        <div className="mv2-container" style={containerNarrow}>
          <MDXProse>
            <h2>Who writes and reviews this</h2>
            <p>
              HireStepX's interview and salary content is written and maintained by the HireStepX product team, cross-checked against public sourcing (candidate-report platforms, company disclosures) rather than a single contributor's memory of one interview loop. Pages are revisited on a rolling basis as new candidate reports and salary data come in; the &ldquo;last reviewed&rdquo; or &ldquo;last verified&rdquo; date on a page is the last time someone on the team re-checked its claims against current sources, not just a template timestamp.
            </p>

            <h2>How we label a claim</h2>
            <p>Every interview question, hiring-process detail, or salary figure on HireStepX falls into one of four labels:</p>
            <ul>
              <li><strong>Officially documented</strong> — stated by the company itself (careers page, official blog, published job description, or a recruiter communication we can point to).</li>
              <li><strong>Candidate-reported</strong> — reported by candidates who went through the actual process, sourced from platforms like AmbitionBox and Glassdoor, or direct submissions. This is the most common category for interview questions and process detail, and it carries the inherent limits of self-reported data: it can be dated, role-specific rather than company-wide, or occasionally inaccurate.</li>
              <li><strong>Common practice for this role</strong> — not tied to one company, but a pattern we can show holds across multiple employers hiring for the same role or focus area (e.g., system-design rounds for senior backend roles generally, not &ldquo;asked at Company X&rdquo;).</li>
              <li><strong>HireStepX practice question</strong> — generated by us to help you rehearse a topic area, not a claim that this exact question was asked anywhere. We never present a practice question as a verified historical interview question.</li>
            </ul>
            <p>
              Pages that don&apos;t yet have enough company-specific data to justify tier-1 or tier-2 attribution say so explicitly rather than borrowing another company&apos;s question set silently — you&apos;ll see this as &ldquo;we don&apos;t yet have {"{"}company{"}"}-specific questions for this focus area&rdquo; language on those pages.
            </p>

            <h2>How salary data is sourced</h2>
            <p>
              Salary bands reflect total CTC (base + variable + equity where applicable), drawn from the 25th–90th percentile of reported offers, sourced primarily from AmbitionBox with Glassdoor as a secondary cross-check, supplemented by role-matched company disclosures where a company has published them directly. We publish a range, not false precision, and we say when we don&apos;t yet have enough sourced offers to report a real percentile for a company — in that case we say so rather than guessing.
            </p>

            <h2>What we don&apos;t do</h2>
            <ul>
              <li>We don&apos;t present a generated practice question as a verified historical interview question.</li>
              <li>We don&apos;t attribute a question set to a company unless our own data actually ties it to that company.</li>
              <li>We don&apos;t reuse one company&apos;s salary data to describe a different company, or one role&apos;s data to describe a different role, even at the same employer.</li>
              <li>We don&apos;t claim more precision than the underlying sample supports.</li>
            </ul>

            <h2>Reporting an inaccuracy</h2>
            <p>
              If something on HireStepX is wrong, outdated, or misattributed, email <CopyEmailLink email="hello@hirestepx.com" /> with the page URL and what looks off. We treat corrections as a priority over new content — a wrong salary figure or misattributed question is worse than a missing page.
            </p>
          </MDXProse>
        </div>
      </section>

      <section className="mv2p-section" aria-label="Start practicing" style={{ ...sectionBase, paddingTop: 8, paddingBottom: 64 }}>
        <div className="mv2-container" style={containerNarrow}>
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            <a
              href="/companies"
